...
class QuoteIn(BaseModel):
    ...
    source_items: Optional[List[Dict[str, Any]]] = None  # Original auto-generated items for comparison
...
class ProjectRequirementsIn(BaseModel):
    ...
    area_m2: float = Field(..., gt=0, description="Area in square meters, must be positive")
    finish_level: FinishLevel = Field(..., description="Desired finish level for the project")
    ...
    material_prefs: List[str] = Field(default_factory=list, description="Material preferences")
...
class GenerationRuleIn(BaseModel):
    ...
    rules: Dict[str, Dict[str, str]] = Field(..., description="Generation rules for labor and materials")
    ...
    def validate_rules_structure(cls, v):
        ...
        for section, section_rules in v.items():
            if section not in allowed_sections:
                raise ValueError(f"Unknown section: {section}. Allowed: {allowed_sections}")
...
class AutoGeneratedItem(BaseModel):
    ...
    confidence_per_item: float = Field(..., description="Confidence level for this item (0.0-1.0)")
...
class AutoGenerateResponse(BaseModel):
    ...
    confidence_per_item: List[float] = Field(..., description="Confidence levels for each item")
