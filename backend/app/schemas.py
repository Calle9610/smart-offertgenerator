...
class GenerationRuleUpdate(BaseModel):
    """
    Schema for updating generation rules.
    
    Only allows updating the rules field.
    """

    rules: Dict[str, Any] = Field(..., description="Updated rule configuration as JSON")


class RuleTestRequest(BaseModel):
    """
    Schema for testing generation rules.
    
    Used by admin endpoints to test rules without saving data.
    """

    key: str = Field(..., description="Rule key to test (e.g., 'bathroom|standard')")
    requirementsData: Dict[str, Any] = Field(..., description="Sample requirements data for testing")
...
class RuleTestResponse(BaseModel):
    ...
    Shows generated items and totals without saving to database.
    """

    items: List[Dict[str, Any]] = Field(..., description="Generated items with quantities and prices")
    subtotal: float = Field(..., description="Calculated subtotal")
    vat: float = Field(..., description="Calculated VAT amount")
    total: float = Field(..., description="Calculated total amount")
...
class PublicQuoteItem(BaseModel):
    ...
    unit_price: Decimal = Field(..., description="Unit price")
    line_total: Decimal = Field(..., description="Line total")
    is_optional: bool = Field(..., description="Whether this item is optional and can be selected/deselected by customer")
    option_group: Optional[str] = Field(None, description="Group for exclusive choices (e.g., 'finish_level', 'extra_features')")
...
class AutoGenerateResponse(BaseModel):
    ...
    subtotal: float = Field(..., description="Subtotal before VAT")
    vat: float = Field(..., description="VAT amount")
    total: float = Field(..., description="Total amount including VAT")
    tuning_applied: List[Dict[str, Any]] = Field(..., description="List of applied tuning factors per item")
    confidence_per_item: Dict[str, str] = Field(..., description="Confidence level per item (low/med/high)")
...
# Option Groups schemas for dynamic quote customization
class OptionGroupItem(BaseModel):
    """Schema for items within an option group."""
    ...
    option_group: str = Field(..., description="Option group name")
    is_selected: bool = Field(False, description="Whether this option is currently selected")


class OptionGroup(BaseModel):
    """Schema for an option group with its items."""
    
    name: str = Field(..., description="Option group name (e.g., 'finish_level', 'extra_features')")
    title: str = Field(..., description="Human-readable title for the option group")
    description: Optional[str] = Field(None, description="Description of what this group represents")
    type: str = Field(..., description="Group type: 'single' for radio buttons, 'multiple' for checkboxes")
    items: List[OptionGroupItem] = Field(..., description="Available options in this group")
    selected_items: List[UUID] = Field(default_factory=list, description="IDs of currently selected items")


class QuoteOptionGroupsResponse(BaseModel):
    """Response schema for quote option groups."""
    
    quote_id: UUID = Field(..., description="Quote ID")
    option_groups: List[OptionGroup] = Field(..., description="Available option groups")
    current_total: Decimal = Field(..., description="Current quote total with selected options")
    base_total: Decimal = Field(..., description="Base quote total without optional items")


class UpdateQuoteOptionsRequest(BaseModel):
    """Request schema for updating quote options."""
    
    selected_items: List[UUID] = Field(..., description="IDs of items to mark as selected")
    deselected_items: List[UUID] = Field(default_factory=list, description="IDs of items to mark as deselected")


class UpdateQuoteOptionsResponse(BaseModel):
    """Response schema for updating quote options."""
