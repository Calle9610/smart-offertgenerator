...
from . import models
from .schemas import UserCreate, TenantCreate, CompanyCreate, ProjectRequirementsCreate, GenerationRuleCreate, QuoteAdjustmentLogCreate
...
def get_quote_by_id_and_tenant(db: Session, quote_id: UUID, tenant_id: UUID) -> Optional[models.Quote]:
...
def create_project_requirements(
    db: Session, 
    requirements: ProjectRequirementsCreate
...
def get_project_requirements_by_company(
    ...
    """
    return db.query(models.ProjectRequirements).filter(
        models.ProjectRequirements.company_id == company_id
    ).order_by(models.ProjectRequirements.created_at.desc()).all()
...
def get_generation_rules_by_company(
    ...
    """
    rules = db.query(models.GenerationRule).filter(
        models.GenerationRule.company_id == company_id
    ).order_by(models.GenerationRule.key).all()
...
def create_quote_adjustment_log(
    db: Session,
    adjustment: QuoteAdjustmentLogCreate
...
def get_adjustment_logs_by_quote(
    ...
    logs = db.query(models.QuoteAdjustmentLog).filter(
