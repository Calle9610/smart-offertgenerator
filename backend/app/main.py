...
from .models import User, Tenant, Company, PriceProfile, LaborRate, Material
...
async def seed_initial_data():
    ...
    try:
        ...
        default_tenant = crud.create_tenant(db, schemas.TenantCreate(
        ...
        default_company = crud.create_company(db, schemas.CompanyCreate(
            name="Default Company AB",
            tenant_id=default_tenant.id
        ))
...
async def create_quote(
    ...
    quote_id = crud.create_quote(db, str(current_user.tenant_id), str(current_user.id), {
    ...
    return {"id": quote_id, "subtotal": float(subtotal), "vat": float(vat), "total": float(total)}
...
async def get_price_profiles(
    ...
    profiles = db.query(PriceProfile).filter(
...
async def create_project_requirements(
    ...
    db_requirements = crud.create_project_requirements(db, schemas.ProjectRequirementsCreate(
...
@app.get("/project-requirements/{requirements_id}", response_model=schemas.ProjectRequirementsOut)
async def get_project_requirements_by_id(
    ...
    requirements = crud.get_project_requirements_by_id(db, uuid.UUID(requirements_id), company_id)
    if not requirements:
        raise HTTPException(
...
@app.put("/project-requirements/{requirements_id}", response_model=schemas.ProjectRequirementsOut)
...
async def delete_project_requirements(
    ...
    success = crud.delete_project_requirements(db, uuid.UUID(requirements_id), company_id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Project requirements not found"
...
@app.get("/quotes/{quote_id}/project-requirements", response_model=schemas.ProjectRequirementsOut)
async def get_project_requirements_by_quote(
    ...
    requirements = crud.get_project_requirements_by_quote(db, uuid.UUID(quote_id), company_id)
    if not requirements:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def create_generation_rule(
    ...
    db_rule = crud.create_generation_rule(db, schemas.GenerationRuleCreate(
        company_id=company_id,
        key=rule.key,
        rules=rule.rules
    ))
...
async def auto_generate_quote(
    ...
    if not profile:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Price profile not found"
        )
        ...
        if 'labor' in generation_rule.rules:
            for ref, expression in generation_rule.rules['labor'].items():
                ...
                if labor_rate:
                    ...
                    unit_price = 0.0
                    description = f"Labor: {ref} (saknar pris - lägg till i labor_rates tabellen)"
                    unit = "hour"
                    line_total = 0.0
                    confidence = 0.3  # Low confidence if rate not found
                
                generated_items.append(schemas.AutoGeneratedItem(
                    kind="labor",
                    ref=ref,
...
async def generate_pdf(
    ...
    try:
        ...
        profile = db.query(PriceProfile).filter(PriceProfile.id == quote.profile_id).first()
...
def log_quantity_changes(
    ...
    for final_item in final_items:
        ...
        if old_qty != new_qty:
            # Log the change
            crud.create_quote_adjustment_log(db, schemas.QuoteAdjustmentLogCreate(
                quote_id=quote_id,
                item_ref=ref,
                old_qty=old_qty,
                new_qty=new_qty,
