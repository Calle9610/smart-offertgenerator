...
class RuleEvaluator:
    ...
    def _evaluate_ternary(self, expression: str) -> Decimal:
        ...
        if colon_pos_in_value == -1:
            raise ValueError(
                f"Invalid ternary expression (no : in value part): {expression}"
            )
    ...
    def _evaluate_arithmetic(self, expression: str) -> Union[int, float]:
        """
        ...
        while "(" in expr:
            ...
            inner_result = self._evaluate_arithmetic(expr[start + 1 : end])
            expr = expr[:start] + str(inner_result) + expr[end + 1 :]

        # Handle multiplication and division
        while "*" in expr or "/" in expr:
            # Find the first * or / operator
            mul_pos = expr.find("*") if "*" in expr else len(expr)
            div_pos = expr.find("/") if "/" in expr else len(expr)
            ...
            left_num = self._extract_number_left(expr, op_pos)
            right_num = self._extract_number_right(expr, op_pos)
            ...
            expr = (
                expr[: op_pos - len(str(left_num))]
                + str(result)
                + expr[op_pos + len(str(right_num)) + 1 :]
            )

        # Handle addition and subtraction
        while "+" in expr or "-" in expr:
            # Find the first + or - operator (but not at the beginning)
            add_pos = expr.find("+", 1) if "+" in expr[1:] else len(expr)
            sub_pos = expr.find("-", 1) if "-" in expr[1:] else len(expr)
    ...
    def _extract_number_left(self, expr: str, pos: int) -> float:
        """Extract the number to the left of a position."""
        start = pos - 1
        while start >= 0 and (expr[start].isdigit() or expr[start] == "."):
            start -= 1
        return float(expr[start + 1 : pos])

    def _extract_number_right(self, expr: str, pos: int) -> float:
        """Extract the number to the right of a position."""
        end = pos + 1
        while end < len(expr) and (expr[end].isdigit() or expr[end] == "."):
            end += 1
        return float(expr[pos + 1 : end])


def evaluate_generation_rule(
    rule_expression: str, requirements: ProjectRequirementsIn
) -> Decimal:
    """
