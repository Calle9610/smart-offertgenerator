...
from app.schemas import FinishLevel, GenerationRuleIn, ProjectRequirementsIn, RoomType
...
class TestRuleEvaluator:
    ...
    def test_ternary_conditions(self):
        ...
        assert evaluator.evaluate_expression("hasElectricalWork ? 4 : 0") == Decimal(
            "0"
        )
        ...
        assert evaluator.evaluate_expression(
            "hasPlumbingWork ? (6 + 2) : 0"
        ) == Decimal("8")
        assert evaluator.evaluate_expression(
            "hasElectricalWork ? 0 : (4 + 1)"
        ) == Decimal("5")
...
class TestGenerationRulesEndpoints:
    ...
    def test_create_generation_rule_without_auth(self):
        ...
        rule_data = {
            ...
            "rules": {
                "labor": {"SNICK": "8 + 2*areaM2", "VVS": "hasPlumbingWork ? 6 : 0"},
                "materials": {"KAKEL20": "areaM2 * 1.2", "FOG5": "ceil(areaM2 / 10)"},
            },
        }
    ...
    def test_invalid_generation_rule_key(self):
        ...
        # Missing separator
        with pytest.raises(ValueError):
            GenerationRuleIn(key="bathroom", rules={"labor": {"SNICK": "8"}})

        # Too many separators
        with pytest.raises(ValueError):
            GenerationRuleIn(
                key="bathroom|standard|extra", rules={"labor": {"SNICK": "8"}}
            )

        # Invalid room type
        with pytest.raises(ValueError):
            GenerationRuleIn(key="invalid|standard", rules={"labor": {"SNICK": "8"}})

        # Invalid finish level
        with pytest.raises(ValueError):
            GenerationRuleIn(key="bathroom|invalid", rules={"labor": {"SNICK": "8"}})

    def test_invalid_generation_rule_structure(self):
        """Test invalid generation rule structure."""
        # Empty rules
        with pytest.raises(ValueError):
            GenerationRuleIn(key="bathroom|standard", rules={})

        # Invalid section
        with pytest.raises(ValueError):
            GenerationRuleIn(
                key="bathroom|standard", rules={"invalid_section": {"SNICK": "8"}}
            )

        # Empty section rules
        with pytest.raises(ValueError):
            GenerationRuleIn(key="bathroom|standard", rules={"labor": {}})
...
class TestAutoGenerationEndpoint:
    ...
    def test_auto_generate_without_auth(self):
        ...
        request_data = {"requirements_id": str(uuid4()), "profile_id": str(uuid4())}
    ...
    def test_auto_generate_missing_fields(self):
        ...
        # Missing profile_id
        request_data = {"requirements_id": str(uuid4())}
...
class TestAutoGenerationSchemas:
    ...
    def test_auto_generate_request_validation(self):
        ...
        valid_request = AutoGenerateRequest(
            requirements_id=str(uuid4()), profile_id=str(uuid4())
        )
    ...
    def test_auto_generate_response_validation(self):
        ...
        from app.schemas import AutoGeneratedItem, AutoGenerateResponse
