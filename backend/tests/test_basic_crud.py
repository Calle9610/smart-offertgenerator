...
class TestBasicCRUD:
    ...
    def test_company_tenant_relationship(self, db_session: Session):
        ...
        companies = db_session.query(Company).filter(Company.tenant_id == tenant.id).all()
...
class TestMultiTenantIsolation:
    ...
    def test_tenant_isolation(self, db_session: Session):
        ...
        # Verify isolation
        tenant1_companies = db_session.query(Company).filter(Company.tenant_id == tenant1.id).all()
        tenant2_companies = db_session.query(Company).filter(Company.tenant_id == tenant2.id).all()
