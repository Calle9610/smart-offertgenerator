...
from app.models import Company, LaborRate, Material, PriceProfile
...
def create_basic_prices():
    ...
    try:
        ...
        for rate in labor_rates:
            print(
                f"   - {rate.code}: {rate.description} ({rate.unit_price} SEK/{rate.unit})"
            )

        print(f"✅ Created {len(materials)} materials:")
        for material in materials:
            print(
                f"   - {material.sku}: {material.name} ({material.unit_cost} SEK/{material.unit})"
            )
